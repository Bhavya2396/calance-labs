...
function TrafficVisualization({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 200 150" className="w-full h-full">
      {/* Grid */}
      {[40, 80, 120, 160].map((x) => (
        <motion.line key={x} x1={x} y1="0" x2={x} y2="150" stroke="rgba(0,0,0,0.3)" strokeWidth="1" />
      ))}
      {[30, 60, 90, 120].map((y) => (
        <motion.line key={y} x1="0" y1={y} x2="200" y2={y} stroke="rgba(0,0,0,0.3)" strokeWidth="1" />
      ))}
      
      {/* Roads */}
      <motion.path d="M0 75 L200 75" stroke={active ? 'rgba(0,0,0,0.5)' : 'rgba(0,0,0,0.3)'} strokeWidth="20"
        initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.8 }} />
      <motion.path d="M100 0 L100 150" stroke={active ? 'rgba(0,0,0,0.5)' : 'rgba(0,0,0,0.3)'} strokeWidth="20"
        initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.8, delay: 0.2 }} />
      
      {/* Intersection */}
      <motion.rect x="85" y="60" width="30" height="30" fill={active ? '#ffffff' : 'rgba(0,0,0,0.4)'} rx="4"
      ...
      {[[50, 50], [150, 50], [50, 100], [150, 100]].map(([x, y], i) => (
        <motion.circle key={i} cx={x} cy={y} r="8" fill={active ? '#ffffff' : 'rgba(0,0,0,0.4)'}
          initial={{ scale: 0 }} animate={{ scale: 1, opacity: active ? [0.6, 1, 0.6] : 1 }}
      ...
      {active && (
        <>
          <motion.rect width="12" height="6" fill="#000000" rx="1"
            initial={{ x: -15, y: 72 }} animate={{ x: 200 }}
            transition={{ duration: 3, repeat: Infinity, ease: 'linear' }} />
          <motion.rect width="6" height="12" fill="#000000" rx="1"
  ...
  return (
    <svg viewBox="0 0 200 150" className="w-full h-full">
      ...
      <motion.rect x="30" y="40" width="140" height="80" rx="8" fill="none" 
        stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="2"
        initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.8 }} />
      
      {/* Gears */}
      <motion.circle cx="70" cy="80" r="20" fill="none" stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="2"
        animate={{ rotate: active ? 360 : 0 }} transition={{ duration: 4, repeat: Infinity, ease: 'linear' }} />
      <motion.circle cx="120" cy="80" r="15" fill="none" stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="2"
      ...
      {active && (
        <>
          <motion.path d="M150 50 Q160 60 150 70 Q140 80 150 90" fill="none" stroke="#000000" strokeWidth="1.5"
            initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.5, delay: 0.8 }} />
          <motion.circle cx="158" cy="55" r="3" fill="#000000" initial={{ scale: 0 }} animate={{ scale: [0, 1, 0] }}
            transition={{ duration: 1.5, repeat: Infinity, delay: 1 }} />
        </>
      )}
      
      {/* Status indicator */}
      <motion.rect x="40" y="130" width="120" height="8" rx="4" fill="rgba(0,0,0,0.2)" />
      <motion.rect x="40" y="130" rx="4" height="8" fill={active ? '#ffffff' : 'rgba(0,0,0,0.5)'}
  ...
  return (
    <svg viewBox="0 0 200 150" className="w-full h-full">
      ...
      <motion.path 
        ...
        fill="none" stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="2"
        initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1.5 }} />
      
      {/* Data points */}
      {active && [[50, 50], [60, 100], [110, 50], [120, 100]].map(([x, y], i) => (
        <motion.circle key={i} cx={x} cy={y} r="4" fill="#000000"
          initial={{ scale: 0 }} animate={{ scale: [0, 1.2, 1] }} transition={{ delay: 0.5 + i * 0.2 }} />
      ))}
      
      {/* AI analysis box */}
      <motion.rect x="140" y="20" width="50" height="40" rx="4" fill="none"
        stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="1.5"
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} />
      <motion.text x="165" y="35" fill={active ? '#c9956c' : 'rgba(0,0,0,0.6)'} fontSize="8" textAnchor="middle"
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}>AI</motion.text>
      <motion.text x="165" y="50" fill={active ? '#c9956c' : 'rgba(0,0,0,0.6)'} fontSize="8" textAnchor="middle"
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.3 }}>94%</motion.text>
      
      {/* Connection line */}
      {active && (
        <motion.path d="M130 75 L140 40" fill="none" stroke="#000000" strokeWidth="1" strokeDasharray="4"
          initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ delay: 1.5 }} />
      )}
      
      {/* Bottom stats */}
      {[20, 60, 100, 140].map((x, i) => (
        <motion.rect key={i} x={x} y={120} width="30" height="20" rx="3" fill={active && i < 3 ? 'rgba(0,0,0,0.4)' : 'rgba(0,0,0,0.2)'}
          stroke={active ? 'rgba(0,0,0,0.5)' : 'rgba(0,0,0,0.3)'} strokeWidth="1"
...
function EcommerceVisualization({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 200 150" className="w-full h-full">
      {/* Chat bubbles */}
      <motion.rect x="10" y="20" width="70" height="35" rx="8" fill={active ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.2)'}
        stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="1.5"
        initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.3 }} />
      <motion.rect x="120" y="60" width="70" height="35" rx="8" fill={active ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.2)'}
        stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="1.5"
        initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.3, delay: 0.3 }} />
      <motion.rect x="10" y="100" width="70" height="35" rx="8" fill={active ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.2)'}
        stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="1.5"
        initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.3, delay: 0.6 }} />
      
      {/* Text lines */}
      {[28, 35, 42].map((y, i) => (
        <motion.rect key={i} x="18" y={y} width={40 - i * 8} height="3" rx="1" fill={active ? '#c9956c' : 'rgba(0,0,0,0.6)'}
          initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.4 + i * 0.1 }} style={{ transformOrigin: 'left' }} />
      ))}
      {[68, 75, 82].map((y, i) => (
        <motion.rect key={i} x="128" y={y} width={45 - i * 10} height="3" rx="1" fill={active ? '#c9956c' : 'rgba(0,0,0,0.6)'}
          initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.7 + i * 0.1 }} style={{ transformOrigin: 'left' }} />
      ))}
      
      {/* AI brain */}
      <motion.circle cx="100" cy="75" r="20" fill={active ? 'rgba(0,0,0,0.4)' : 'rgba(0,0,0,0.2)'}
        stroke={active ? '#ffffff' : 'rgba(0,0,0,0.5)'} strokeWidth="1.5"
      ...
      {active && (
        <>
          <motion.path d="M80 37 L85 65" stroke="#000000" strokeWidth="1" strokeDasharray="3"
            initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ delay: 0.9 }} />
          <motion.path d="M120 77 L115 77" stroke="#000000" strokeWidth="1" strokeDasharray="3"
            initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ delay: 1 }} />
          <motion.path d="M80 117 L85 85" stroke="#000000" strokeWidth="1" strokeDasharray="3"
